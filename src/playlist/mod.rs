...
use gio::Application;
use gtk::{
    ApplicationWindow, CellLayoutExt, CellRendererPixbuf, CellRendererText, DialogExt, FileChooserExt, FileFilterExt, ListStore, ListStoreExt, ListStoreExtManual, StaticType, ToValue, TreeIter, TreeModelExt, TreeSelectionExt, TreeView, TreeViewColumn, TreeViewColumnExt, TreeViewExt, Type, WidgetExt
} ;
...
impl Playlist { 
    ...
    fn set_pixbuf(&self , row  : &TreeIter  , tag : &Tag){ 
        if let Some(picture) = tag.pictures().next() { 
            ...
            if let Some(pixbuf) = pixbuf_loader.get_pixbuf() {
                ...
                self.list_store.set_value(row, THUMBNAIL_COLUMN, &thumbnail.to_value());
                self.list_store.set_value(row, PIXBUF_COLUMN, &pixbuf.to_value());
    ...
    pub fn add(&self , path : &Path) {
        ...
        if let Ok(tag) = Tag::read_from_path(path)  {
            ...
            let year = tag.year().map(|y| y.to_string()).unwrap_or_else(|| "(no year)".to_string()) ;
            let track = tag.track().map(|t| t.to_string()).unwrap_or_else(|| "(no track)".to_string()) ;
            let total_tracks = tag.total_tracks().map(|t| t.to_string()).unwrap_or_else(|| "(no total tracks)".to_string()) ;
            let track_value = format!("{} of {}", track, total_tracks) ;

            self.set_pixbuf(&row, &tag) ; 
            self.list_store.set_value(&row, TITLE_COLUMN, &title.to_value());
            self.list_store.set_value(&row, ARTIST_COLUMN, &artist.to_value());
            self.list_store.set_value(&row, ALBUM_COLUMN, &album.to_value());
            self.list_store.set_value(&row, GENRE_COLUMN, &genre.to_value());
            self.list_store.set_value(&row, YEAR_COLUMN, &year.to_value());
            self.list_store.set_value(&row, TRACK_COLUMN, &track_value.to_value());
    }else {
        self.list_store.set_value(&row, TITLE_COLUMN, &filename.to_value());

    }

    let path = path.to_str().unwrap_or_default() ;
    self.list_store.set_value(&row, PATH_COLUMN, &path.to_value());
    
}

pub fn show_open_dialog(parent : &ApplicationWindow) -> Option<PathBuf> { 
    let mut file = None ; 
    let dialog = FileChooserDialog::new(Some(("Select an MP3 file")) , Some(parent) , FileChooserAction::Open) ;
